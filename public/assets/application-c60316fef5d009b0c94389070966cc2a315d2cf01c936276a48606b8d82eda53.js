...
(function() {
  ...
  roundup = function(float, exp) {
    var multiplier;
    multiplier = Math.pow(10, exp);
    return (Math.ceil(float * multiplier) / multiplier).toFixed(exp);
  };

  ready = function() {
    var FitPrice, brandjsonobj, relabeljsonobj;
    relabeljsonobj = 0;
    $.ajax({
      url: "/relabel.json",
      dataType: "json",
      success: function(response) {
        return relabeljsonobj = response;
      }
    });
    $('#quotation_relabel_quantity').keyup(function() {
      $('#quotation_relabel_charge').val(0);
      return relabeljsonobj.relabels.forEach(function(relabel) {
        if ($('#quotation_relabel_quantity').val() >= relabel.start_quantity && $('#quotation_relabel_quantity').val() <= relabel.end_quantity) {
          if (relabeljsonobj.currency === 'SGD') {
            return $('#quotation_relabel_charge').val(roundup($('#quotation_relabel_quantity').val() * relabel.relabel_charge_sg, 2));
          } else {
            return $('#quotation_relabel_charge').val(roundup($('#quotation_relabel_quantity').val() * relabel.relabel_charge, 2));
          }
        }
      });
    });
    $('#quotation_woven_tag_quantity').keyup(function() {
      $('#quotation_woven_tag_charge').val(0);
      return relabeljsonobj.woven_tags.forEach(function(woven_tag) {
        if ($('#quotation_woven_tag_quantity').val() >= woven_tag.start_quantity && $('#quotation_woven_tag_quantity').val() <= woven_tag.end_quantity) {
          if (relabeljsonobj.currency === 'SGD') {
            return $('#quotation_woven_tag_charge').val(roundup($('#quotation_woven_tag_quantity').val() * woven_tag.woven_tag_charge_sg, 2));
          } else {
            return $('#quotation_woven_tag_charge').val(roundup($('#quotation_woven_tag_quantity').val() * woven_tag.woven_tag_charge, 2));
          }
        }
      });
    });
    $('#quotation_sew_tag_quantity').keyup(function() {
      $('#quotation_sew_tag_charge').val(0);
      if (relabeljsonobj.currency === 'SGD') {
        return $('#quotation_sew_tag_charge').val(roundup($('#quotation_sew_tag_quantity').val() * (1.0 / relabeljsonobj.exchange_rate), 2));
      } else {
        return $('#quotation_sew_tag_charge').val(roundup($('#quotation_sew_tag_quantity').val() * 1.0, 2));
      }
    });
    $('.selectable-row').on('click', 'td:not(.not-show)', function() {
      return window.location = $(this).parent().data('href');
    });
    FitPrice = 0;
    $("#quotation_back_print_method_id").change(function() {
      var print_method_name;
      ...
      return $.ajax({
        ...
        dataType: "json",
        success: function(response) {
          return response.forEach(function(p) {
            var $test_html;
            $test_html = $("<div></div>");
            if (print_method_name.toString() === p.print_method_name.toString()) {
              return p.block_sizes.forEach(function(b) {
                return $("#quotation_back_block_size_id").append($('<option>', {
    ...
    $("#quotation_front_print_method_id").change(function() {
      var print_method_name;
      ...
      return $.ajax({
        ...
        success: function(response) {
          return response.forEach(function(p) {
            ...
            if (print_method_name.toString() === p.print_method_name.toString()) {
              return p.block_sizes.forEach(function(b) {
                return $("#quotation_front_block_size_id").append($('<option>', {
                  value: b.block_size_id,
                  text: b.block_size_name
                }));
              });
      ...
      return $.ajax({
        ...
        success: function(response) {
          if (response.currency === 'SGD') {
            ...
            return $("#quotation_max_rrp").val(response.max_rrp_sg);
          } else {
            $("#quotation_price").val(response.price);
            $("#quotation_cost").val(response.cost);
            $('#quotation_min_rrp').val(response.min_rrp);
            return $('#quotation_max_rrp').val(response.max_rrp);
          }
        }
      });
    });
    $("#quotation_shirt_type_id").change(function() {
      var shirt_type_name;
      ...
      return $.ajax({
        ...
        dataType: "json",
        success: function(response) {
          return response.shirt_types.forEach(function(shirt_type) {
            var $test_html;
            $test_html = $("<div></div>");
            if (shirt_type_name.toString() === shirt_type.shirt_type_name.toString()) {
              return shirt_type.fits.forEach(function(fit) {
                return $("#quotation_fit_id").append($('<option>', {
                  value: fit.fit_id,
                  text: fit.fit_name
                }));
              });
    ...
    $("#quotation_brand_id").change(function() {
      var brand_name;
      ...
      return $.ajax({
        ...
        dataType: "json",
        success: function(response) {
          return response.forEach(function(brand) {
            var $test_html;
            $test_html = $("<div></div>");
            if (brand_name.toString() === brand.brand.toString()) {
              ...
              return brand.shirt_types.forEach(function(shirt_type) {
                return $('#quotation_shirt_type_id').append($('<option>', {
